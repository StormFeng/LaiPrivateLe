...
public class MainPagerTabView extends FrameLayout {
	...
	public MainPagerTabView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void init() {
		...
		FrameLayout.LayoutParams lineParams = new FrameLayout.LayoutParams((int) (TDevice.dpToPixel(150) / titles.size()), (int) TDevice.dpToPixel(3));
	...
	public void setCurrentTab(int currentTabIndex) {
		...
		if (titles != null) {
			ViewHelper.setTranslationX(line_iv, TDevice.dpToPixel(150) / titles.size() * currentTabIndex);
		}
	}

	public void onPageScrolled(int arg0, float arg1, int arg2) {
		ViewHelper.setTranslationX(line_iv, TDevice.dpToPixel(150) / titles.size() * (arg0 + arg1));
	}
}
