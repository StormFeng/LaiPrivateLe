...
public class DynamicActiveFragment extends BaseGridListFragment<MGroup> implements OnGroupListener {
    ...
    GroupBroadcastReceiver groupBroadcastReceiver = new GroupBroadcastReceiver() {
        public void onReceive(android.content.Context context, android.content.Intent intent) {
            ...
            if (GroupBroadcastReceiver.ACTION_ACTIVITY_SNYC_OVER.equals(intent.getAction())) {
                ...
                // 加入或新建活动
            } else if (GroupBroadcastReceiver.ACTION_JOIN_ACTIVITY.equals(intent.getAction())) {
                MGroup mGroup = DaoOperate.getInstance(_activity).queryLastMGroup(Constant.gType_activity);
                if (mGroup != null) {
                ...
                // 退出或解散活动
            } else if (GroupBroadcastReceiver.ACTION_EXIT_ACTIVITY.equals(intent.getAction())) {
            ...
            } else if (GroupBroadcastReceiver.ACTION_UPDATE_ACTIVITY.equals(intent.getAction())) {//更新资料
                String groupId = intent.getStringExtra("groupId");
                for (MGroup mGroup : resultList) {
                    if (mGroup.getGroupId().equals(groupId)) {
    ...
    protected IDataSource<MGroup> getDataSource() {
        return new DynamicActiveListDataSource(_activity);
    }
    ...
    public void onGroup(MGroup mGroup) {
        //TODO
        if (Constant.gType_activity.equals(mGroup.getGroupType())) {
