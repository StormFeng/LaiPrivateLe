...
public class DynamicZanListTpl extends BaseTpl<ObjectWrapper> {
    @Bind(R.id.zanListLayout)
    LinearLayout zanListLayout;

    public DynamicZanListTpl(Context context) {
        super(context);
    }
    ...
    public void setBean(ObjectWrapper bean, int position) {
        ...
        if (list != null) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if ((TDevice.dpToPixel(30f * (i + 2)) + TDevice.dpToPixel(10 * (i + 1)) + TDevice.dpToPixel(40)) > (TDevice.getScreenWidth())) {
                    break;
                }
            }
        }
