...
public class VoteActiveDetailCommentTpl extends BaseTpl<Object> implements OnSendClickListener {
    ...
    public VoteActiveDetailCommentTpl(Context context) {
        super(context);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.item_vote_active_detail_comment;
    }
                ...
                if (dialog == null) {
                    ...
                    dialog.setOnActionClickListener(new OnActionClickListener() {
                        ...
                        public void onActionClick(ActionDialog dialog, View View, int position) {
                            ApiClient.getApi().deleteComment(VoteActiveDetailCommentTpl.this, ac.getLoginUid(), bean.getId());
                        }
                    });
                }
                dialog.show();
    ...
    private void addNewComment(String commentId) {
        ...
        newComment.setItemViewType(VoteActiveDetailActivity.TPL_COMMENT);
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            // 如果为点赞，评论，收藏条目
            if (obj.getItemViewType() == VoteActiveDetailActivity.TPL_ACTIONBAR) {
            ...
            // 如果为“没有评论”条目
            if (obj.getItemViewType() == VoteActiveDetailActivity.TPL_COMMENT_EMPTY) {
                // 移除“没有评论条目”
                data.remove(obj);
                // 添加新条目
            ...
            // 如果为“评论”条目
            if (obj.getItemViewType() == VoteActiveDetailActivity.TPL_COMMENT) {
                // 添加新条目
                data.add(i, newComment);
                break;
            }
            ...
            if (API_TAG_COMMENT.equals(tag)) {// 添加评论
                ...
                for (int i = 0; i < data.size(); i++) {
                    ...
                    if (obj.getItemViewType() == VoteActiveDetailActivity.TPL_ACTIONBAR) {
                        ...
                        // 如果评论为空,添加“没有评论“条目
                        if (newComCount == 0) {
                            data.add(new ObjectWrapper("没有评论", VoteActiveDetailActivity.TPL_COMMENT_EMPTY));
                        }
                        break;
                    }
                }
