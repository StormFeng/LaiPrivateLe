...
public class DynamicDetailCommentTpl extends BaseTpl<Object> implements OnSendClickListener {
    ...
    public DynamicDetailCommentTpl(Context context) {
        super(context);
    }
                ...
                if (dialog == null) {
                    ...
                    dialog.setOnActionClickListener(new OnActionClickListener() {
                        ...
                        public void onActionClick(ActionDialog dialog, View View, int position) {
                            ApiClient.getApi().deleteComment(DynamicDetailCommentTpl.this, ac.getLoginUid(), bean.getId());
                        }
                    });
                }
                dialog.show();
    ...
    public void onSendClick(String content) {
        ...
        ApiClient.getApi().comment(this, ac.getLoginUid(), _activity.getIntent().getExtras().getString(Const.BUNDLE_KEY_DYNAMIC_ID), bean.getId(), "2", content);
    }
        ...
        if (res.isOK()) {
            if (API_TAG_COMMENT.equals(tag)) {// 添加评论
                ...
                for (int i = 0; i < data.size(); i++) {
                    ...
                    if (obj.getItemViewType() == DynamicDetailActivity.TPL_ACTIONBAR) {
                        DynamicBean dynamicBean = (DynamicBean) obj;
                        int newComCount = (Integer.parseInt(dynamicBean.getComCount()) - 1);
                        dynamicBean.setComCount(newComCount + "");
                        // 如果评论为空,添加“没有评论“条目
                        if (newComCount == 0) {
                            data.add(new ObjectWrapper("没有评论", DynamicDetailActivity.TPL_COMMENT_EMPTY));
                        }
                        break;
                    }
                }
