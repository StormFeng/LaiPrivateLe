...
public class ModifyNameActivity extends BaseActivity implements TextWatcher {
    ...
    private void initView() {
        topbar.setTitle("姓名").setLeftText("取消", UIHelper.finish(this)).setRightText("保存").getRight_tv().setEnabled(false);
        ;
        if (_Bundle != null) {
    ...
    public void submit() {
        ApiClient.getApi().changePersonInfo(this, ac.getLoginUid(), null, null, null, null, input_et.getText().toString().trim(), null, null, null);
    }
    ...
    public void onApiSuccess(Result res, String tag) {
        ...
        if (res.isOK()) {
            ac.setProperty(Const.USER_NICKNAME, input_et.getText().toString().trim());
            setResult(RESULT_OK);
            finish();
        } else {
