...
public class DynamicActiveListDataSource extends BaseListDataSource<MGroup> {

	public DynamicActiveListDataSource(Context context) {
		super(context);
	}
	...
	protected ArrayList<MGroup> load(int page) throws Exception {
		...
		hasMore = false;
		if(ac.isLogin()) {
			return (ArrayList<MGroup>) DaoOperate.getInstance(_activity).queryMGroups(Constant.gType_activity);
		}else{
