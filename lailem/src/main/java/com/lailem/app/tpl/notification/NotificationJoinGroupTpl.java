...
public class NotificationJoinGroupTpl extends BaseTpl<Message> {
    @Bind(R.id.state)
    TextView stateTV;
    @Bind(R.id.title)
    TextView titleTV;
    ...
    public NotificationJoinGroupTpl(Context context) {
        super(context);
    }
    ...
    protected int getLayoutId() {
        ...
        return R.layout.item_notification_join_group;
    }
    ...
    public void setBean(Message bean, int position) {
        ...
        MsgJoinG msgObj = bean.getMsgObj();
        HInfo hInfo = msgObj.gethInfo();
        String way = msgObj.getWay();
        if (Constant.gType_group.equals(msgObj.getgInfo().getgType())) {
            if (hInfo != null && Constant.gJoinWay_invite.equals(way)) {
                titleTV.setText(msgObj.gethInfo().getNick() + "邀请" + msgObj.getuInfo().getNick() + "加入了群组");
            } else {
                titleTV.setText(msgObj.getuInfo().getNick() + "加入了群组");
            }
        } else {
            if (hInfo != null && Constant.gJoinWay_invite.equals(way)) {
                titleTV.setText(msgObj.gethInfo().getNick() + "邀请" + msgObj.getuInfo().getNick() + "参加了活动");
            } else {
                titleTV.setText(msgObj.getuInfo().getNick() + "参加了活动");
            }
        }
        contentTV.setText("新人报到，请多多包涵！");
