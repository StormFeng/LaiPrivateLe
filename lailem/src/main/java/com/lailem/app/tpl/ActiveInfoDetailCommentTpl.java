...
public class ActiveInfoDetailCommentTpl extends BaseTpl<Object> implements OnSendClickListener {
    ...
    public ActiveInfoDetailCommentTpl(Context context) {
        super(context);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.item_active_info_detail_comment;
    }
                ...
                if (dialog == null) {
                    ...
                    dialog.setOnActionClickListener(new OnActionClickListener() {
                        ...
                        public void onActionClick(ActionDialog dialog, View View, int position) {
                            ApiClient.getApi().deleteComment(ActiveInfoDetailCommentTpl.this, ac.getLoginUid(), bean.getId());
                        }
                    });
                }
                dialog.show();
    ...
    private void addNewComment(String commentId) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            // 如果为点赞，评论，收藏条目
            if (obj.getItemViewType() == ActiveInfoDetailActivity.TPL_DETAIL) {
            ...
            if (API_TAG_COMMENT.equals(tag)) {// 添加评论
                ...
                for (int i = 0; i < data.size(); i++) {
                    ...
                    if (obj.getItemViewType() == ActiveInfoDetailActivity.TPL_DETAIL) {
                        ...
                        // 如果评论为空,添加“没有评论“条目
                        if (newComCount == 0) {
                            data.add(new ObjectWrapper("没有评论", ActiveInfoDetailActivity.TPL_COMMENT_EMPTY));
                        }
                        break;
                    }
                }
